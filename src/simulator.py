	...
	def run_sim(self, duration:int=10, camera:str='camera_rgb'):
		...
		with mujoco.viewer.launch_passive(self.model, self.model_data) as viewer:
			...
			while viewer.is_running() and time.time() - sim_start_time < duration:
				...
				self.model_data.ctrl = self.controller.get_control(self.model_data, running_time=time.time()-sim_start_time)
				#step the simulation
				...
				i += 1
				self.renderer.update_scene(self.model_data, camera='camera_rgb')
				cam_pixels = self.renderer.render()
				camera_frames.append(cam_pixels)
		media.show_video(camera_frames, fps=self.fps)
